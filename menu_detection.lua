...
local blue_magic_visible_until = 0;
...
local function chat_input_open()
    ...
    if (not ok or open == nil) then return false; end
...
local function blue_magic_menu_name(name)
    ...
    if (name:find('bluequip', 1, true) or
            ...
            name:find('blueset', 1, true) or
            name:find('bluinven', 1, true)) then
        return true;
    end
...
local function blue_magic_setting_visible(name)
    if (blue_magic_menu_name(name)) then
        blue_magic_visible_until = os.clock() + 0.75;
        return true;
    end
    if (os.clock() < blue_magic_visible_until and neutral(name)) then
        return true;
    end
    blue_magic_visible_until = 0;
    return false;
end
...
function menu.is_main_open()
    ...
    if (blue_magic_setting_visible(name)) then
        main_menu_latched = false;
        return false;
    end
...
function menu.should_hide()
    ...
    local name = menu.get_name():lower();
    if (blue_magic_setting_visible(name)) then return false; end
