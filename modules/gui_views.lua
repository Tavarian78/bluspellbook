...
local function draw_job_traits()
    ...
    local spell_levels = {};
    for _, spell in ipairs(state.spells) do
        local key = spell_key(spell.name);
    ...
    for name, equipped in pairs(equipped_names) do
        if (equipped) then
            equipped_spells[spell_key(name)] = true;
        end
    end

    local function get_all_contributors(trait, maximum_level)
        local contributors = {};
        for value in trait[3]:gmatch('[^,]+') do
            local item = value:match('^%s*(.-)%s*$');
            local name = item:gsub('%s*%(%d+%)%s*$', '');
            local key = spell_key(name);
            local spell_level = spell_levels[key];
            if (spell_level ~= nil and spell_level <= maximum_level) then
                local points = tonumber(item:match('%((%d+)%)%s*$')) or 1;
                contributors[#contributors + 1] = {
                    item = ('%s - %d %s'):fmt(
                        name, points, points == 1 and 'pt' or 'pts'
                    ),
                    name = name,
                    key = key,
                    priority = required_spells[name] and 1 or
                        (recommended_spells[name] and 2 or 3),
                    level = spell_level,
                };
            end
        end
    ...
    local function count_trait_points(trait, spells, maximum_level)
        ...
        for value in trait[3]:gmatch('[^,]+') do
            ...
            local name = item:gsub('%s*%(%d+%)%s*$', '');
            local key = spell_key(name);
            if (spells[key] and spell_levels[key] ~= nil and
                    spell_levels[key] <= maximum_level) then
    ...
    local function get_trait_tier(trait, points)
        local tier = 0;
    ...
    local function tier_available_level(trait, requirement)
        local contributors = {};
        for value in trait[3]:gmatch('[^,]+') do
            local item = value:match('^%s*(.-)%s*$');
            local name = item:gsub('%s*%(%d+%)%s*$', '');
            local level = spell_levels[spell_key(name)];
            if (level ~= nil) then
                contributors[#contributors + 1] = {
                    level = level,
        ...
        for _, contributor in ipairs(contributors) do
            ...
            if (points >= requirement) then
                return math.max(trait[2], contributor.level);
            end
        end
        return nil;
    end
    ...
    if (equipped_available) then
        ...
        imgui.TextColored({ 1.0, 0.82, 0.16, 1.0 },
    ...
    imgui.SetColumnWidth(1, selected_trait == 0 and 500 * scale or 220 * scale);
    imgui.PushTextWrapPos(
        imgui.GetCursorPosX() + imgui.GetColumnWidth() - (10 * scale)
    );
    imgui.TextColored({ 1.0, 0.72, 0.30, 1.0 }, 'Trait and Current Tier');
    imgui.PopTextWrapPos();
    imgui.NextColumn();
    imgui.TextColored({ 1.0, 0.72, 0.30, 1.0 }, 'Tier Requirements');
    ...
    for trait_index, trait in ipairs(job_traits) do
        ...
        local known_points = count_trait_points(trait, known_spells, player_level);
        local equipped_points = equipped_available and
            count_trait_points(trait, equipped_spells, player_level) or 0;
        local active_tier = get_trait_tier(trait, equipped_points);
        ...
        if (visible) then
            ...
            if (active_tier > 0) then
                if (#trait[4] > 1) then
                    trait_label = ('%s %s (%d pts)'):fmt(
                        trait[1], tier_text(active_tier), equipped_points
                    );
                else
                    trait_label = ('%s (%d pts)'):fmt(trait[1], equipped_points);
                end
                imgui.TextColored({ 0.35, 1.0, 0.45, 1.0 }, trait_label);
            elseif (not can_acquire) then
                imgui.TextColored({ 1.0, 0.28, 0.28, 1.0 }, trait_label);
            else
                imgui.TextColored({ 1.0, 0.82, 0.16, 1.0 }, trait_label);
            end
            imgui.PopTextWrapPos();
            imgui.NextColumn();
            imgui.PushTextWrapPos(
            ...
            for tier_index, requirement in ipairs(trait[4]) do
                local available_level = tier_available_level(trait, requirement);
                local requirement_text = available_level ~= nil and
                    ('%s: %d pts - Lv.%d'):fmt(
                        tier_text(tier_index), requirement, available_level
                    ) or
                    ('%s: %d pts'):fmt(tier_text(tier_index), requirement);
                if (active_tier >= tier_index) then
                    imgui.TextColored({ 0.35, 1.0, 0.45, 1.0 }, requirement_text);
                elseif (available_level ~= nil and
                        player_level >= available_level and
                        known_points >= requirement) then
                    imgui.TextColored({ 1.0, 0.82, 0.16, 1.0 }, requirement_text);
                else
                    imgui.TextColored({ 1.0, 0.28, 0.28, 1.0 }, requirement_text);
                end
            end
            imgui.PopTextWrapPos();
            if (selected_trait ~= 0) then
                ...
                for _, spell in ipairs(get_all_contributors(
                        trait, contributor_level)) do
                    local text = spell.item;
                    if (equipped_spells[spell.key]) then
                        imgui.TextColored({ 0.35, 0.75, 1.0, 1.0 }, text);
                    elseif (known_spells[spell.key]) then
                        imgui.TextColored({ 0.35, 1.0, 0.45, 1.0 }, text);
                    else
                        imgui.TextColored({ 1.0, 0.28, 0.28, 1.0 }, text);
                    end
                end
                imgui.PopTextWrapPos();
            end
