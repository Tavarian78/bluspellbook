...
local function draw_job_traits()
    ...
    local trait_filter_changed = imgui.Combo(
        '##trait_level_filter',
        state.trait_level_filter,
        'Active\0Available\0All\0\0'
    );
    if (trait_filter_changed and state.trait_level_filter[1] == 1) then
    ...
    for trait_index, trait in ipairs(job_traits) do
        ...
        for tier_index, requirement in ipairs(trait[4]) do
            ...
            if (available_level ~= nil and player_level >= available_level and
                    known_points >= requirement) then
                available_tier = tier_index;
            end
        end
        ...
        elseif (filter_index == 1) then
            visible = visible and available_tier > 0;
        end
        if (visible) then
            local can_acquire = available_tier > 0;
            ...
            for tier_index, requirement in ipairs(trait[4]) do
                if (filter_index ~= 1 or tier_index <= available_tier) then
                    local available_level = job_traits.available_level(
                        trait, spell_levels, requirement
                    );
                    local requirement_text = available_level ~= nil and
                        ('%s: %d pts - Lv.%d'):fmt(
                            job_traits.tier_text(tier_index), requirement,
                            available_level
                        ) or
                        ('%s: %d pts'):fmt(
                            job_traits.tier_text(tier_index), requirement
                    ...
                    elseif (tier_index <= available_tier) then
