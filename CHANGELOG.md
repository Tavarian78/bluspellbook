...
- Fixed the main window hiding while typing in the chat box.
- Fixed Auto Refresh activating before the required 8 trait points.
- Kept the addon visible while setting Blue Magic, even when menu auto-hide is enabled.
- Colored mob-list spells green when learned, yellow when learnable, and red when locked.
- Stopped showing trait tiers that the listed spells cannot provide enough points to reach.
- Added the level 42 override for Auroral Drape so it appears and counts toward Fast Cast.
- Fixed missing row separators for traits that have no obtainable tier.
- Fixed the addon hiding during Blue Magic spell-selection transitions.
- Selecting Available traits now resets the trait-name filter to All Traits.
- The Available trait filter now hides locked traits and locked tiers.
...
- not maintained was on another server and was personal use only
